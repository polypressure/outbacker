...
Why bother with eliminating this conditional logic from your controllers? Far too often, this logic spirals out of control, attracts business logic, and becomes increasingly cumbersome and fragile as we add code to handle new cases. And unfortunately, the process controls teams put in place to enforce good code hygiene chronically break down in the face of schedule pressure, growing teams, unrevisited emergency fixes, etc.

It turns out that not only is Outbacker a prophylaxis against fat, complicated controllers, it supports a very simple, low-ceremony way to write intention-revealing Rails code with both skinny controllers _and_ skinny models. If you feel these are worthwhile aims for your Ruby/Rails code—but you've found many approaches to accomplish this ineffective or not worth the trouble—then you might find Outbacker valuable.

## Typical Rails Controllers
...
class AppointmentsController < ApplicationController
  ...
  def create
    calendar.book_appointment(appointment_params) do |on_outcome|
      ...
      on_outcome.of(:insufficient_credits) do |appointment|
        redirect_to new_credits_path,
                    alert: "You don't have enough credits, please purchase more."
      end
...
An outcome callback block can take any number of arguments, passed on from the business-logic method. Here, we pass a single Appointment object representing the appointment that has been booked.
...
class AppointmentsController < ApplicationController
  ...
  def create
    ...
    calendar.book_appointment(appointment_params) do |on|
      ...
      on.outcome_of_insufficient_credits do |appointment|
        redirect_to new_credits_path,
                    alert: "You don't have enough credits, please purchase more."
      end
  ...
  def book_appointment(params, &outcome_handlers)
    with(outcome_handlers) do |outcomes|
      ...
      if user_lacks_sufficient_credits?
        outcomes.handle :insufficient_credits, appointment
        return
      end
...
In short, this says to process the outcome of :insufficient_credits with the corresponding handler callback passed in via the outcome_handlers block. Again as a side benefit, this makes the intent of the code explicit: we unambiguously can see that we've determined the outcome of the method at this point, and what exactly that outcome is.
